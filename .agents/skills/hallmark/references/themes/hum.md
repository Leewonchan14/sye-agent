...
- "Learn anything. In short sessions. With friends."
- "Streaks work. We just made them friendlier."
- "PostHog ships. Here's this week."
- "Turns out, tracking your habits can be fun."

Body copy is natural, short-sentence, with space for clarity. No marketing fluff.

- "5 minutes a day. That's it."
- "When you finish a streak, the app throws confetti. Yes, really."
- "We don't track engagement. We track *curiosity*."

Never: "supercharge", "unlock", "revolutionise", "seamless", "journey", "paradigm", "next-gen", "bleeding-edge", "delightful" (as a slide-bullet). The product *is* delightful — don't tell, show.

## Anti-patterns

- **No pure black `#000` on anything** — ink is `oklch(20% 0.012 250)` (cool near-black).
- **No pure white `#fff`** — paper is cream `oklch(97% 0.012 95)`.
- **No glassmorphism** — the rounded-corners-and-frosted-glass stack belongs elsewhere.
- **No serif anywhere** — Hum's identity is entirely rounded sans.
- **No emoji as icons** — every mark is CSS/SVG.
- **No accent-to-accent gradients** — three accents, unmixed.
- **No centred hero stack by default** — leads the deadliest-tell list. Default off-centre.
- **No badge above H1** (the "✨ now with X" tell).
- **No three-equal-card rows with the same accent** — vary the shape or the distribution.
- **No floating accent-stripe on a card edge** — reads AI-generated.
- **No all-caps grey eyebrow above everything** — use sparingly.
- **No decorative gradient on background + headline + button at once** — Hum bans accent-to-accent gradients; never triple-stack one.

## How Hum differs from neighbouring themes

| vs | difference |
|---|---|
| **Coral** (modern-minimal) | Coral is warm-grey + single coral accent + Geist + restrained motion + quiet typography. Hum is cream + three accents + rounded-sans + multi-surface animation + playful voice. Same product-landing genre, completely different temperature and energy. |
| **Bloom** (atmospheric) | Bloom is warm-light atmospheric (rose or oat cream, editorial serif, fade-in reveals, quiet). Hum is bright-cream pear-yellow, rounded sans, spring-hover motion, loud multi-accent. Bloom breathes; Hum bounces. |
| **Atelier** (atmospheric) | Atelier is dark, italic-serif, and centered on a *maker* identity — craft, portfolio, small studio. Hum is light, rounded-sans, and centered on a *user* identity — tools for curiosity, learning, friendship. Different audience, different paper. |

## Test brief expectations

Hum should be a candidate when the brief mentions:

- *habit tracker · streak · curiosity · learning · daily · friendship · kids · creative tool · indie software · small product · playful · warm · fun · alive · social · gamified*
- Product categories: *learning platform · habit app · friendship app · kids' tool · creative software · indie product · daily-curiosity app · community-driven tool*
- Emotional tone: *warm · playful · alive · friendly · casual · smart-direct · not-too-serious*

Briefs for enterprise / B2B / security / compliance / dev-tool route to Cobalt or Coral. Briefs for editorial / magazine / long-form route to Atelier or Newsprint. Hum is for products that want to feel like the room is warm and someone smart is smiling.

## Build hint

The first lines of CSS to establish Hum's anchor moves:

```css
html, body { overflow-x: clip; }
body { background: var(--color-paper); color: var(--color-ink); font-family: var(--font-body); }

/* Highlight system — em-relative band, wraps with text */
em, .hl {
  background-image: linear-gradient(var(--hl, oklch(86% 0.18 95 / 0.55)) 0 0);
  background-repeat: no-repeat; background-size: 100% 0.32em; background-position: 0 82%;
  -webkit-box-decoration-break: clone; box-decoration-break: clone;
  font-style: normal; padding-bottom: 0.02em;
}

/* Button push system (full definition in #1 above) */
.btn { /* …copy the full .btn block from signature #1… */ }

/* Section band shell model (full definition in layout discipline above) */
:root { --shell: var(--page-max); }
```
